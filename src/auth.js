...
export function registerAuthentication(app, { getDb, env = process.env }) {
  ...
  app.get('/api/auth/status', async (req, res) => {
    ...
    res.json({ configured: cfg.configured, authenticated: Boolean(auth), role: auth?.session?.role || null });
  });
  ...
  app.post('/api/auth/pin-login', async (req, res) => {
    ...
    res.json({ ok: true, authenticated: true, role: 'ADMIN', expiresAt });
  });
  ...
  app.use('/api', async (req, res, next) => {
    try {
      ...
      if (!SAFE_METHODS.has(req.method) && (req.body?.forza === true || req.query?.forza === 'true')) {
        return res.status(403).json({ error: 'Operazione forzata bloccata: richiede MFA step-up' });
      }
